...
export default function ProductsTab({ products, category, onAdd, onSell, onDelete }: ProductsTabProps) {
  const [showAddModal, setShowAddModal] = useState(false);
  const [search, setSearch] = useState('');

  const categoryLabel = category === 'parts' ? 'قطع الغيار' : 'الأدوات الإلكترونية';
  const categoryIcon = category === 'parts' ? '⚙️' : '🔧';
    ...
    <div className="flex flex-col h-full">
      ...
      {products.length > 0 && (
        <div className="mt-4 bg-gradient-to-l from-blue-600 to-blue-700 text-white rounded-xl p-3 flex items-center justify-between shadow-sm">
          ...
          <span className="font-bold text-lg">{totalValue.toLocaleString()} د.ع</span>
        </div>
      )}
